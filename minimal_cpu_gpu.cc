...
void* job_cpu(tflite::InterpreterBuilder* builder, vector<cv::Mat> input){
   ...
   cpu_t = average_time / (float)SEQ;
   return 0;
 }


void* job_gpu(tflite::InterpreterBuilder* builder, vector<cv::Mat> input){
   std::unique_ptr<tflite::Interpreter> interpreter;
   (*builder)(&interpreter);
   TFLITE_MINIMAL_CHECK(interpreter != nullptr);
...
int main(int argc, char* argv[]) {
  ...
  cpu = std::thread(&job_cpu, &builder, input);
  gpu = std::thread(&job_gpu, &builder, input);
