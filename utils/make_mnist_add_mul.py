...
from tensorflow.keras.layers import Dense, Add,Multiply,Input,Conv2D, Dropout, Flatten, MaxPooling2D, concatenate 
...
class model_GPU(Model):
    def __init__(self):
        ...
        self.conv1 = Conv2D(10,(3,3),activation=tf.nn.elu)
        self.pool1 = MaxPooling2D(2,2)
        self.conv2 = Conv2D(20,(3,3),activation=tf.nn.elu)
        self.pool2 = MaxPooling2D(2,2)
        self.conv3 = Conv2D(40,(3,3),activation=tf.nn.elu)
        self.flat = Flatten()
        self.dense1 = Dense(128, activation=tf.nn.relu)
        self.dense2 = Dense(10, activation = tf.nn.softmax)
        self.conv_adder = Conv2D(10,(3,3), activation =tf.nn.elu)
        self.conv_mulr = Conv2D(10,(3,3), activation =tf.nn.elu)
        #self.dummy2 = Conv2D(5,(3,3), activation==tf.nn.elu)
        #self.dummy3 = Conv2D(5,(3,3), activation==tf.nn.elu)
    def call(self, x):
        adder = self.conv_adder(x)
        net = self.conv1(x)
        net = Add()([net,adder]) #HOON : concatenate -> add layer
        mulr = self.conv_mulr(x)
        net = Multiply()([net,mulr])
        net = self.pool1(net)
        net = self.conv2(net)
        #model_dummy_2 = iself.dummy2(tf.zeros([1,13,13,1]))
...
myGpuModel.save('saved_model/mnist_7')
